...
from observer.scan import hash_file
...
def make_detector(tmp_path: Path) -> tuple[ChangeDetector, RecordingObserver]:
    observer = RecordingObserver(tmp_path)
    detector = ChangeDetector(observer, tmp_path)
...
def test_first_poll_of_new_path_fires_no_event_and_not_yet_known(tmp_path):
    detector, observer = make_detector(tmp_path)
...
def test_second_poll_same_size_fires_created_and_records_known_entry(tmp_path):
    ...
    assert observer.events == [(EventType.CREATED, str(rel))]
...
def test_size_differs_between_first_and_second_poll_restarts_stabilization(tmp_path):
    ...
    detector.poll([relp])
    assert observer.events == [(EventType.CREATED, str(relp))]
...
def test_modify_content_same_size_fires_modified_without_stability_wait(tmp_path):
    ...
    assert observer.events == [(EventType.MODIFIED, str(relp))]
...
def test_removing_known_file_fires_deleted_and_removes_entry(tmp_path):
    ...
    assert observer.events == [(EventType.DELETED, str(relp))]
    assert relp not in detector._known
...
def test_two_independent_paths_stabilize_and_change_independently(tmp_path):
    ...
    detector.poll([slow_rel, fast_rel])
    assert observer.events == [(EventType.CREATED, str(fast_rel))]
    ...
    detector.poll([slow_rel, fast_rel])
    assert observer.events == [(EventType.CREATED, str(slow_rel))]
...
def test_seed_with_report_existing_fires_found_per_path(tmp_path):
    ...
    assert observer.events == [(EventType.FOUND, str(a)), (EventType.FOUND, str(b))]
    assert detector._known[a] == hash_file(tmp_path / a)
    assert detector._known[b] == hash_file(tmp_path / b)
...
def test_poll_after_seed_same_size_modification_fires_modified_immediately(tmp_path):
    ...
    assert observer.events == [(EventType.MODIFIED, str(a))]
...
# --- hash-error-handling ---
# Assumes detector.py imports hash_file via `from observer.scan import hash_file`,
# so it can be monkeypatched at "observer.detector.hash_file".
...
def test_new_path_permission_error_fires_error_and_leaves_pending(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("observer.detector.hash_file", _raise_permission_error)
    detector.poll([a])

    assert observer.events == [(EventType.ERROR, str(a))]
...
def test_new_path_permission_error_then_success_fires_created(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("observer.detector.hash_file", hash_file)
    detector.poll([a])

    assert observer.events == [(EventType.CREATED, str(a))]
...
def test_seed_permission_error_on_one_path_still_seeds_others(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("observer.detector.hash_file", selective_raise)
    detector.seed([a, b])
...
def test_new_path_deleted_mid_hash_fires_created_then_deleted(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("observer.detector.hash_file", _raise_file_not_found)
    detector.poll([a])

    assert observer.events == [(EventType.CREATED, str(a)), (EventType.DELETED, str(a))]
    assert a not in detector._known
    assert a not in detector._pending
...
def test_known_path_changed_size_deleted_mid_hash_fires_modified_then_deleted(
    ...
    assert observer.events == [(EventType.MODIFIED, str(a)), (EventType.DELETED, str(a))]
    assert a not in detector._known
    assert a not in detector._pending
...
def test_known_path_unchanged_size_deleted_mid_hash_fires_deleted_only(tmp_path, monkeypatch):
    ...
    assert observer.events == [(EventType.DELETED, str(a))]
    assert a not in detector._known
    assert a not in detector._pending
