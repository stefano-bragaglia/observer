...
class ChangeDetector:
    def __init__(self, observer: Observer, folder: Path) -> None:
    ...
    def seed(self, paths: list[Path], *, report_existing: bool = False) -> None:
        for path in paths:
            try:
                ...
                self._observer._notify(EventType.ERROR, str(path))
                continue
            self._known[path] = (digest, size)
            if report_existing:
                self._observer._notify(EventType.FOUND, str(path))
    ...
    def _poll_new_path(self, path: Path, abs_path: Path, current_size: int) -> None:
        ...
        try:
            ...
            self._observer._notify(EventType.CREATED, str(path))
            self._observer._notify(EventType.DELETED, str(path))
            return
        except OSError:
            self._observer._notify(EventType.ERROR, str(path))
            return
    ...
    def _poll_known_path(self, path: Path, abs_path: Path, current_size: int) -> None:
        ...
        try:
            ...
            del self._pending[path]
            self._observer._notify(EventType.MODIFIED, str(path))
